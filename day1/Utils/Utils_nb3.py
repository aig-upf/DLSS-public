...
def figure5(sbs_logistic, sbs_nn):
    ...
    axs[0].plot(sbs_nn.losses, 'b', label='linear-layer Network - Training', alpha=.5)
    axs[1].plot(sbs_nn.val_losses, 'r', label='linear-layer Network - Validation', alpha=.5)
...
def weights_comparison(w_logistic_output, w_nn_equiv):
    ...
    ax0.bar(np.arange(25), w_nn_equiv.cpu().numpy().squeeze(), alpha=.5, label='linear-layer Network (Composed)')
    ...
    ax1.set_ylabel('linear-layer network (Composed)')
